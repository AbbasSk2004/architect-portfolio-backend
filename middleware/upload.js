...
export const uploadBlogCoverMulter = uploadBlogCover.single('coverImage')

// Middleware to upload blog cover to Cloudinary
export const uploadBlogCoverToCloudinary = async (req, res, next) => {
  try {
    if (!req.file) {
      req.uploadedBlogCover = null
      return next()
    }
    ...
    const uploadPromise = new Promise((resolve, reject) => {
      ...
      const publicId = `blog_cover_${timestamp}_${originalName}`
      
      const uploadOptions = {
        folder: 'architect-portfolio/blogs',
        resource_type: 'image',
        public_id: publicId,
        quality: 'auto',
      }
